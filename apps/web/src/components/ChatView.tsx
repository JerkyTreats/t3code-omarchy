...
import { MinimalChatComposer } from "./MinimalChatComposer";
...
export default function ChatView({ threadId, conversationPanel = null }: ChatViewProps) {
  const serverThread = useThreadById(threadId);
  ...
  const { deleteProjectScript, saveProjectScript, updateProjectScript } =
    useProjectScriptActions(activeProject);
  ...
  useEffect(() => {
    ...
    if (!script) {
      toastManager.add({
        type: "error",
        title: "Saved script no longer exists",
      });
      return;
    }
      ...
      <div className="flex min-h-0 min-w-0 flex-1">
        ...
        >
          ...
          <div className="relative flex min-h-0 flex-1 flex-col">
            ...
            {!conversationPanel && showScrollToBottom && (
          ...
          <div
            className={cn(
              ...
              conversationPanel ? "border-t border-border/40" : "",
            )}
          >
            <form
              ref={composerFormRef}
