...
const SidebarProjectItem = memo(function SidebarProjectItem(props: SidebarProjectItemProps) {
  ...
  const handleProjectButtonClick = useCallback(
    ...
    [
      clearSelection,
      dragInProgressRef,
      openProjectManagementForMember,
  ...
  const handleProjectToggleClick = useCallback(
    (event: React.MouseEvent<HTMLButtonElement>) => {
      event.preventDefault();
      event.stopPropagation();
      if (dragInProgressRef.current) {
    ...
    [clearSelection, dragInProgressRef, project.projectKey, toggleProject],
  );
    ...
    <>
      <div className="group/project-header relative">
        <button
          ...
          className="absolute top-1 left-1 z-10 inline-flex size-5 cursor-pointer items-center justify-center rounded-md text-muted-foreground/70 transition-colors hover:bg-secondary hover:text-foreground focus-visible:outline-hidden focus-visible:ring-1 focus-visible:ring-ring"
        ...
        <SidebarMenuButton
          ...
          size="sm"
          className={`gap-2 py-1.5 pr-8 pl-7 text-left hover:bg-accent group-hover/project-header:bg-accent group-hover/project-header:text-sidebar-accent-foreground ${
