...
function createDesktopBridgePersistenceStub(
  overrides: Partial<LocalApi["persistence"]> = {},
): Window["desktopBridge"] {
  return {
    getWsUrl: () => "ws://localhost/",
    getClientSettings: overrides.getClientSettings ?? (async () => null),
    setClientSettings: overrides.setClientSettings ?? (async () => undefined),
    getSavedEnvironmentRegistry: overrides.getSavedEnvironmentRegistry ?? (async () => []),
    setSavedEnvironmentRegistry: overrides.setSavedEnvironmentRegistry ?? (async () => undefined),
    getSavedEnvironmentSecret: overrides.getSavedEnvironmentSecret ?? (async () => null),
    setSavedEnvironmentSecret: overrides.setSavedEnvironmentSecret ?? (async () => true),
    removeSavedEnvironmentSecret: overrides.removeSavedEnvironmentSecret ?? (async () => undefined),
...
describe("environment runtime catalog stores", () => {
  beforeEach(async () => {
    vi.stubGlobal("window", {
      ...
      desktopBridge: createDesktopBridgePersistenceStub(),
    });
