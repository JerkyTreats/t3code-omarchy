import type { EnvironmentApi, ThreadId } from "@t3tools/contracts";

import { newCommandId } from "./lib/utils";

interface StopThreadRuntimeInput {
  api: Pick<EnvironmentApi, "orchestration" | "terminal">;
...
interface ReleaseDedicatedWorktreeInput {
  api: Pick<EnvironmentApi, "vcs" | "orchestration" | "terminal">;
  clearTerminalState?: (() => void) | undefined;
...
export async function releaseDedicatedWorktree({
  ...
  if (desiredBranch) {
    try {
      await api.vcs.switchRef({ cwd: repoRoot, refName: desiredBranch });
      branchActivatedInPrimary = true;
    } catch {
      const fallbackPrimaryStatus = await api.vcs
        .refreshStatus({ cwd: repoRoot })
        .catch(() => null);
      nextPrimaryBranch = fallbackPrimaryStatus?.refName ?? null;
    }
  }
