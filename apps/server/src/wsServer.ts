...
> {
  ...
  const routeRequest = Effect.fnUntraced(function* (request: WebSocketRequest) {
    switch (request.body._tag) {
      ...
      case WS_METHODS.gitMergeBranches: {
        const body = stripRequestTag(request.body);
        return yield* git.mergeBranches(body);
      }

      case WS_METHODS.gitAbortMerge: {
        const body = stripRequestTag(request.body);
        return yield* git.abortMerge(body.cwd);
      }
      ...
      case WS_METHODS.githubCreateIssue: {
        const body = stripRequestTag(request.body);
        return yield* gitHubManager.createIssue(body);
      }
