...
it.layer(NodeServices.layer)("server router seam", (it) => {
  ...
  it.effect("serves orchestration snapshots over HTTP", () =>
    Effect.gen(function* () {
      ...
      const body = (yield* response.json) as ReturnType<typeof makeDefaultOrchestrationReadModel>;

      assert.equal(response.status, 200);
      assert.equal(body.snapshotSequence, snapshot.snapshotSequence);
      assert.deepEqual(body.projects, snapshot.projects);
      assert.deepEqual(body.threads, snapshot.threads);
    }).pipe(Effect.provide(NodeHttpServer.layerTest)),
  );
