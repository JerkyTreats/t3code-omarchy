...
export const makeForkGitDomain = Effect.gen(function* () {
  ...
  const createProgressEmitter = (
    ...
    const emit = (payload: GitActionProgressPayload) => {
      if (!options?.progressReporter) {
        return Effect.void;
      }
      return options.progressReporter.publish({
        ...payload,
        actionId,
        cwd: input.cwd,
        action: input.action,
  ...
  const runCommitStep = (
    ...
    filePaths?: readonly string[],
    _progressReporter?: GitActionProgressReporter,
    _actionId?: string,
  ) =>
    Effect.gen(function* () {
      const suggestion =
  ...
  const runPromoteStep = (
    ...
    Effect.gen(function* () {
      ...
      const sourceCommitAndPush = {
        commit,
        push: yield* gitCore.pushCurrentBranch(cwd, sourceBranch),
      };
...
export const ForkGitDomainLive = Layer.effect(ForkGitDomain, makeForkGitDomain);
