  ...
  WsRpcGroup.toLayer(
    Effect.gen(function* () {
      ...
      const loadServerConfig = Effect.gen(function* () {
        ...
        const environment = yield* Effect.service(ServerEnvironment).pipe(
      ...
      return WsRpcGroup.of({
        ...
        [WS_METHODS.gitMergeBranches]: (input) =>
          observeRpcEffect(
            ...
            gitManager.mergeBranches(input).pipe(Effect.tap(() => refreshGitStatus(input.cwd))),
            { "rpc.aggregate": "git" },
          ),
        [WS_METHODS.gitAbortMerge]: (input) =>
          observeRpcEffect(
            WS_METHODS.gitAbortMerge,
            gitManager.abortMerge(input).pipe(Effect.tap(() => refreshGitStatus(input.cwd))),
            { "rpc.aggregate": "git" },
          ),
        ...
        [WS_METHODS.gitRepositoryContext]: (input) =>
          observeRpcEffect(WS_METHODS.gitRepositoryContext, gitManager.repositoryContext(input), {
            "rpc.aggregate": "git",
          }),
        [WS_METHODS.githubStatus]: (input) =>
          observeRpcEffect(WS_METHODS.githubStatus, gitHubCli.getStatus(input), {
            "rpc.aggregate": "github",
          }),
        [WS_METHODS.githubLogin]: (input) =>
          observeRpcEffect(WS_METHODS.githubLogin, gitHubCli.login(input), {
            "rpc.aggregate": "github",
          }),
        [WS_METHODS.githubListIssues]: (input) =>
          observeRpcEffect(WS_METHODS.githubListIssues, gitHubCli.listIssues(input), {
            "rpc.aggregate": "github",
          }),
        [WS_METHODS.githubCreateIssue]: (input) =>
          observeRpcEffect(WS_METHODS.githubCreateIssue, gitHubCli.createIssue(input), {
            "rpc.aggregate": "github",
          }),
        [WS_METHODS.githubCloseIssue]: (input) =>
          observeRpcEffect(WS_METHODS.githubCloseIssue, gitHubCli.closeIssue(input), {
            "rpc.aggregate": "github",
          }),
        [WS_METHODS.githubReopenIssue]: (input) =>
          observeRpcEffect(WS_METHODS.githubReopenIssue, gitHubCli.reopenIssue(input), {
            "rpc.aggregate": "github",
          }),
...
function hasModernWebSocketCredential(request: HttpServerRequest.HttpServerRequest): boolean {
  const url = HttpServerRequest.toURL(request);
  const webSocketToken = Option.isSome(url) ? url.value.searchParams.get("wsToken") : null;
  if (typeof webSocketToken === "string" && webSocketToken.trim().length > 0) {
    return true;
  }

  if (typeof request.cookies[SESSION_COOKIE_NAME] === "string") {
    return true;
  }
  ...
  Effect.succeed(
    HttpRouter.add(
      ...
      Effect.gen(function* () {
        ...
        const makeRpcWebSocketHttpEffect = (sessionId: AuthSessionId) =>
          RpcServer.toHttpEffectWebsocket(WsRpcGroup, {
            spanPrefix: "ws.rpc",
            spanAttributes: {
            ...
            Effect.provide(Layer.mergeAll(makeWsRpcLayer(sessionId), RpcSerialization.layerJson)),
          );

        if (!hasModernWebSocketCredential(request)) {
          return HttpServerResponse.text("Unauthorized WebSocket connection", { status: 401 });
        }

        const session = yield* serverAuth.authenticateWebSocketUpgrade(request);
        const rpcWebSocketHttpEffect = yield* makeRpcWebSocketHttpEffect(session.sessionId);
